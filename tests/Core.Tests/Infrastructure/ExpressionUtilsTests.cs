...
using Conjunction.Core.Tests.Model;
using FluentAssertions;
using Xunit;

namespace Conjunction.Core.Tests.Infrastructure
{
  public class ExpressionUtilsTests
  {
    ...
    public void GetPropertySelector_PropertyNameIsValid_ReturnsPropertySelectorExpression()
    {
      ...
      Expression<Func<TestIndexableEntity, object>> expectedPropertySelector = (x => x.SomeString);
    ...
    {
      // Arrange
      Expression<Func<TestIndexableEntity, object>> propertySelector = (x => x.SomeString);

      // Act
      var actual = ExpressionUtils.GetPropertyNameFromPropertySelector(propertySelector);

      // Assert
      actual.ShouldBeEquivalentTo("SomeString");
    }
    ...
    [Theory]
    [InlineData("SomeString", typeof(string))]
    [InlineData("SomeDateTime", typeof(DateTime))]
