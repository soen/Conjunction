...
namespace Conjunction.Core.Infrastructure
{
  internal sealed class MutableDependencyResolver : IMutableDependencyResolver
  {
    ...
    {
      _registrations[typeof(TService)] = () => GetInstance(typeof(TImpl));
    }

    public void Register<TService>(Func<TService> instanceFactory)
    {
      _registrations[typeof(TService)] = () => instanceFactory();
    }
    ...
    {
      var lazy = new Lazy<TService>(instanceFactory, LazyThreadSafetyMode.ExecutionAndPublication);
      Register(() => lazy.Value);
    }
